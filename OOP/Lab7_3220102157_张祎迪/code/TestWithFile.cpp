...
{
public:
	vecWrapperT(int type, std::vector<T, Alloc::MyAllocator<T> > *pVec)
	{
		m_type = type;
		m_pVec = pVec;
	}
	virtual ~vecWrapperT() {
		if (m_pVec)
			delete ((std::vector<T, Alloc::MyAllocator<T> > *)m_pVec);
	}
public:
	virtual void visit(int index)
	{
		T temp = (*(std::vector<T, Alloc::MyAllocator<T> > *)m_pVec)[index];
	}
	virtual int size()
	{
		return ((std::vector<T, Alloc::MyAllocator<T> > *)m_pVec)->size();
	}
	virtual void resize(int size)
	{
		((std::vector<T, Alloc::MyAllocator<T> > *)m_pVec)->resize(size);
	}
	...
	{
		(*(std::vector<T, Alloc::MyAllocator<T> > *)m_pVec)[index] = *((T *)value);
	}
};
...
int main()
{
	...
	for (int i = 0; i < TESTSIZE; i++)
	{
		...
		{
			start = clock();
			vecWrapperT<int> *pNewVec = new vecWrapperT<int>(INT, new std::vector<int, Alloc::MyAllocator<int> >(tSize));
		...
		{
			start = clock();
			vecWrapperT<float> *pNewVec = new vecWrapperT<float>(INT, new std::vector<float, Alloc::MyAllocator<float> >(tSize));
			testVec[i] = (vecWrapper *)pNewVec;
			stop = clock();
			runtime += (double)(stop - start) / CLOCKS_PER_SEC;
		}
		...
		{
			start = clock();
			vecWrapperT<double> *pNewVec = new vecWrapperT<double>(INT, new std::vector<double, Alloc::MyAllocator<double> >(tSize));
			testVec[i] = (vecWrapper *)pNewVec;
			stop = clock();
			runtime += (double)(stop - start) / CLOCKS_PER_SEC;
		}
		...
		{
			start = clock();
			vecWrapperT<myObject> *pNewVec = new vecWrapperT<myObject>(CLASS, new std::vector<myObject, Alloc::MyAllocator<myObject> >(tSize));
			testVec[i] = (vecWrapper *)pNewVec;
			stop = clock();
			runtime += (double)(stop - start) / CLOCKS_PER_SEC;
		}
