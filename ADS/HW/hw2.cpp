...
bool comp_func (node *p1, node *p2){return p1->key[0] < p2->key[0];}
node *Root = new node();
...
void split(node *treenode){
    ...
    sort(parent->child,parent->child+parent->child_size,comp_func);
    split(parent);
}
...
void insert(node *leaf,int value){
    ...
    sort(leaf->key,leaf->key+leaf->key_size);
    split(leaf);
}
...
void print_tree(){
    ...
    while(!level.empty()){
        ...
        cout << "[";
        for (int i = 0; i < temp->key_size; i++){
            if (i){cout<<",";}
            cout << temp->key[i];
        }
        cout << "]";
        for(int i = 0; i < temp->child_size; i++){
            level.push(temp->child[i]);
        }
