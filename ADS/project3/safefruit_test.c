...
void BronKerbosch(int cur, int num, int price){
    for(int i = cur; i>=0; i--){
        /*
            ...
            2. Adding all the fruit remaining can't produce better cost
            3. Adding all the fruit that can be eaten together can't produce better cost
            4. Adding all the fruit remaining is still smaller than the current best choice
        */
        ...
        for(FF* p = F[i+1].next ;p!=NULL; p = p->next){
            if(current_sel[p->id-1]){
                goto next;
            }
        }
        current_sel[i] = 1;
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        conflict[a][b] = 1;
        conflict[b][a] = 1;
        struct FF* p1 = (struct FF*)malloc(sizeof(struct FF));
        struct FF* p2 = (struct FF*)malloc(sizeof(struct FF));
        p1->id = a;
        p2->id = b;
        p1->next = F[b].next;
        F[b].next = p1;
